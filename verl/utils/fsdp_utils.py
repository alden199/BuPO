...
def fsdp2_grad_norm_(parameters, norm_type=2.0, error_if_nonfinite=False, foreach=None):
    """torch.nn.utils.clip_grad_norm_ cann't run on cpu parameter DTensor"""
    from torch.nn.utils.clip_grad import _clip_grads_with_norm_, _get_total_norm
    ...
    if isinstance(parameters, torch.Tensor):
        ...
        # prevent generators from being exhausted
        parameters = list(parameters)
    grads = [p.grad.cpu() for p in parameters if p.grad is not None]
    # grads = []
    # for p in parameters:
    #     if p.grad is not None:
    #         # 确保梯度在正确的设备上
    #         grad = p.grad
    #         if hasattr(grad, '_local_tensor'):  # 处理 DTensor
    #             grad = grad._local_tensor
            
    #         # 如果当前环境没有 CUDA，将梯度移到 CPU
    #         if grad.is_cuda and not torch.cuda.is_available():
    #             grad = grad.cpu()
